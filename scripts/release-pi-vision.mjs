 ...
 * packages, and this directory is not a workspace member.
 *
 * Usage:
 *   node scripts/release-pi-vision.mjs <major|minor|patch|x.y.z> [--dry-run] [--commit]
 *
 * Steps:
 ...
 * 7. Install dependencies if needed, then typecheck inside the package dir
 *    (the package ships TypeScript sources, so this is the build/validation
 *    step; npm install uses --no-workspaces because the pi repo's root
 *    package-lock.json confuses npm's workspace resolution in subdirectories)
...
const PACKAGE_DIR = join(SCRIPT_DIR, "..", "packages", "coding-agent", "extensions", "pi-vision");
const PACKAGE_JSON = join(PACKAGE_DIR, "package.json");
const CHANGELOG = join(PACKAGE_DIR, "CHANGELOG.md");

if (!RELEASE_TARGET || (!BUMP_TYPES.has(RELEASE_TARGET) && !SEMVER_RE.test(RELEASE_TARGET))) {
	console.error("Usage: node scripts/release-pi-vision.mjs <major|minor|patch|x.y.z> [--dry-run] [--commit]");
	process.exit(1);
}
...
if (DRY_RUN) {
	// Dry run: no file writes, no install/typecheck, no publish.
	console.log("Dry run: skipping file writes, install, typecheck, and publish.");
...
// 6. Install dependencies if needed, then typecheck
if (!existsSync(join(PACKAGE_DIR, "node_modules"))) {
	console.log("Installing dependencies (no node_modules present)...");
	// --no-workspaces: the pi repo's root package-lock.json confuses npm's
	// workspace resolution when installing inside a nested extension dir.
	run("npm install --ignore-scripts --no-workspaces", { cwd: PACKAGE_DIR });
	console.log();
}
console.log("Typechecking (tsc --noEmit)...");
run("npm run typecheck", { cwd: PACKAGE_DIR });
console.log();
