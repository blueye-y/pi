...
import { type DescriberDeps, describeSingle, resolveMaxTokens, runBatch } from "../../src/describer.js";
import { getErrorLogPath, type VisionErrorLogEntry } from "../../src/error-log.js";
import { imageHash } from "../../src/image.js";
import { DESCRIPTION_TRUNCATED_MARKER, type ExtractedImage, type VisionConfig } from "../../src/index.js";

// The describer now appends a structured error-log entry on every failure
// (src/error-log.ts → ~/.pi/agent/logs/pi-vision/errors.log). Redirect
...
describe("provider routing", () => {
	...
	it("uses a matching extension provider stream instead of the global compatibility registry", async () => {
		...
		expect(options.sessionId).toMatch(/^pi-vision:[0-9a-f-]{36}$/);
	...
	it("falls back to completeSimple when no matching extension stream is registered", async () => {
		...
		expect(completeSimple.mock.calls[0][2].sessionId).toMatch(/^pi-vision:[0-9a-f-]{36}$/);
	});
});
...
describe("resolveMaxTokens", () => {
	...
	const cfg = (maxTokens?: number): VisionConfig => ({ ...baseCfg, maxTokens });
