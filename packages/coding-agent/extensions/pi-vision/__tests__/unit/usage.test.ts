...
describe("readEnergyFromTee", () => {
	...
	it("returns an empty capture for a stream with no comment lines", async () => {
		const capture = await readEnergyFromTee(sseBody(['data: {"choices": []}', "data: [DONE]"]));
...
describe("buildUsageRecord", () => {
	...
	it("reports energy when tokens are zero but SSE energy was captured", () => {
		...
		const capture: VisionEnergyCapture = {
			...EMPTY_ENERGY_CAPTURE,
			energyJoules: 12.5,
	...
	it("falls back to the vision model id/provider when the response omits them", () => {
		const rec = buildUsageRecord(makeMessage({ model: "", provider: "" }), EMPTY_ENERGY_CAPTURE, makeModel(), "h");
	...
	it("produces a record assignable to VisionUsageRecord", () => {
		const rec: VisionUsageRecord | null = buildUsageRecord(makeMessage(), EMPTY_ENERGY_CAPTURE, makeModel(), "h");
		expect(rec).not.toBeNull();
	});
	...
	it("lists imageHashes only for a genuine batch (length > 1), with the first as representative", () => {
		const rec = buildUsageRecord(makeMessage(), EMPTY_ENERGY_CAPTURE, makeModel(), "h1", ["h1", "h2", "h3"]);
...
describe("fetch interceptor + AsyncLocalStorage routing", () => {
	...
	it("routes each concurrent fetch's teed body to its own ALS store (no cross-contamination)", async () => {
		...
		const mockFetch = vi.fn(async (input: RequestInfo | URL) => sseResponse(bodies[String(input)] ?? ["nope"]));
		globalThis.fetch = mockFetch as typeof globalThis.fetch;
		installFetchInterceptor();
