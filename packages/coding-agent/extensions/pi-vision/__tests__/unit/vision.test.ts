...
describe("normalizeConfig", () => {
	...
	it("rejects negative / non-finite maxDescriptionLines", () => {
		expect(normalizeConfig({ maxDescriptionLines: -3 }).maxDescriptionLines).toBe(DEFAULT_CONFIG.maxDescriptionLines);
...
describe("config round-trip via PI_CODING_AGENT_DIR", () => {
	...
	beforeEach(() => {
		tmpHome = mkdtempSync(join(tmpdir(), "pi-vision-"));
	...
	it("writes to ~/.pi-equivalent/extensions/pi-vision.json", () => {
		const path = writeConfig({ ...DEFAULT_CONFIG, visionModel: "openai/gpt-4o" });
		expect(path).toBe(join(tmpHome, "extensions", "pi-vision.json"));
		expect(existsSync(path)).toBe(true);

		const onDisk = JSON.parse(readFileSync(path, "utf8")) as VisionConfig;
	...
	it("readConfig tolerates a corrupt file", () => {
		...
		writeFileSync(join(dir, "pi-vision.json"), "{not json", "utf8");
		expect(readConfig()).toEqual(DEFAULT_CONFIG);
	});

	it("readConfig migrates the legacy pi-vision-handoff.json config", () => {
		const dir = join(tmpHome, "extensions");
		mkdirSync(dir, { recursive: true });
		const legacy = join(dir, "pi-vision-handoff.json");
		writeFileSync(legacy, JSON.stringify({ ...DEFAULT_CONFIG, visionModel: "anthropic/claude" }), "utf8");
		expect(readConfig().visionModel).toBe("anthropic/claude");
		// The new file now exists and carries the migrated config; the legacy
		// file is left untouched.
		expect(existsSync(join(dir, "pi-vision.json"))).toBe(true);
		expect(existsSync(legacy)).toBe(true);
	});

	it("getConfigPath points at extensions/pi-vision.json", () => {
		expect(getConfigPath()).toBe(join(tmpHome, "extensions", "pi-vision.json"));
	});
});
...
describe("insertImageDescriptions", () => {
	...
	it("inserts a description before every image block across formats, keeping each image", async () => {
		...
		const result = await insertImageDescriptions(input as (TextContent | ImageContent)[], describe);
		expect(result.changed).toBe(true);
		expect(result.content).toEqual([
...
describe("wrapDescription", () => {
	...
	it("applies the configured line cap when maxDescriptionLines > 0", () => {
		const cfg: VisionConfig = { ...DEFAULT_CONFIG, maxDescriptionLines: 2 };
...
describe("insertImageDescriptions with a batched (pre-resolved) describe callback", () => {
	const _base64 = "iVBORw0KGgoAAAANSUhEUgAAAAEAAAAB";
	const textBlock = { type: "text", text: "hello" } as const;
