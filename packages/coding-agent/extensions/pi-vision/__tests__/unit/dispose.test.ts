...
import { abortWireGuard, fetchInterceptorGuard, timeoutGuard } from "../../src/dispose.js";
import { fetchInterceptorRefcount, uninstallFetchInterceptor } from "../../src/usage.js";
...
describe("fetchInterceptorGuard", () => {
	...
	it("installs on construct and uninstalls on dispose via `using`", () => {
		...
		{
			using _guard = fetchInterceptorGuard();
	...
	it("nests refcounts across multiple guards", () => {
		{
			using _a = fetchInterceptorGuard();
			using _b = fetchInterceptorGuard();
