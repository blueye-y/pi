...
const { readImageBufferBoundedMock, findPastedImagePathsMock, readImageBufferMock, resolvePrewarmImageMock } =
	vi.hoisted(() => ({
...
// the test does not depend on the user's ~/.pi/agent/extensions config. The
// hoisted object is mutable so individual tests can toggle clipboardPaste
// before invoking the factory (shortcut registration is config-gated).
const { handoffTestConfig } = vi.hoisted(() => ({
	handoffTestConfig: {
		enabled: true,
		visionModel: "test/vision",
		autoHandoff: true,
		handoffModels: [] as string[],
...
describe("clipboard paste shortcut registration", () => {
	it("registers no paste-key shortcuts (ctrl+v is editor-intercepted; only super+v on macOS)", () => {
		handoffTestConfig.clipboardPaste = true;
		const { pi: piOn } = setup();
		const keys = piOn.registerShortcut.mock.calls.map((c) => c[0]);
		// The platform paste key (ctrl+v) is handled by the editor interception,
		// NOT registered — so no "Extension shortcut conflict" is reported. Only
		// super+v (Command+V, macOS) may be registered, and only on darwin.
		expect(keys).not.toContain("ctrl+v");
		expect(keys).not.toContain("alt+v");
		expect(keys).not.toContain("ctrl+alt+v");
		if (process.platform === "darwin") expect(keys).toEqual(["super+v"]);
		else expect(keys).toEqual([]);

		handoffTestConfig.clipboardPaste = false;
		const { pi: piOff } = setup();
		expect(piOff.registerShortcut).not.toHaveBeenCalled();
	});
});
...
describe("pi-fabric codemode nested tool_result + context hook", () => {
	...
	it("keeps the image block for a normal (non-fabric) toolCallId", async () => {
		...
		expect(result).toBeDefined();
		expect(result!.content.some((b) => b.type === "image")).toBe(true);
		expect(result!.content.some((b) => (b as { text?: string }).text?.includes("does not support images"))).toBe(
			false,
		);
	});
	...
	it("swaps the re-attached fabric_exec image for the description via the context hook", async () => {
		...
		const messages = [
			{
				role: "toolResult",
				content: [{ type: "text", text: "Read image file [image/png]" }, imageContent("AAA")],
			},
		];
		const result = (await handlers.context({ type: "context", messages: messages as any }, ctxWithSignal())) as
	...
	it("substitutes UNAVAILABLE via the context hook when the describer fails", async () => {
		...
		const messages = [
			{
				role: "toolResult",
				content: [{ type: "text", text: "Read image file [image/png]" }, imageContent("ZZZ")],
			},
		];
		const result = (await handlers.context({ type: "context", messages: messages as any }, ctxWithSignal())) as
			| { messages: Array<{ content: Array<{ type: string; text?: string }> }> }
...
describe("async pasted-path fallback race", () => {
	...
	it("renders a dim summary until expanded, then dims the expanded body per line", () => {
		const { pi } = setup();
		const renderer = pi.registerMessageRenderer.mock.calls.find(([customType]) => customType === "vision-async")?.[1];
