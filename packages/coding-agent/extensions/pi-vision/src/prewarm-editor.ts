 ...
 *
 * Opt-in via `VisionConfig.prewarmPastedImages`. When off (or the
...
export interface PrewarmEditorDeps {
	...
	/** Live gate: intercept the platform paste key (ctrl+v) for the marker flow.
	 *  True when `clipboardPaste` is on. When false, the key falls through to
	 *  pi's built-in paste. */
	shouldInterceptPaste: () => boolean;
	/** Run the clipboard paste flow (marker-based) from the editor. The editor
	 *  passes itself so the caller can insert the marker at the cursor. */
	onPasteKey: (editor: PrewarmEditor) => void;
}
...
export class PrewarmEditor extends CustomEditor {
	...
	private readonly shouldInterceptPaste: () => boolean;
	private readonly onPasteKey: (editor: PrewarmEditor) => void;
	// The KeybindingsManager is private in CustomEditor, so keep our own
	// reference from the constructor to check the paste keybinding.
	...
	constructor(tui: TUI, theme: EditorTheme, keybindings: KeybindingsManager, deps: PrewarmEditorDeps) {
		...
		this.shouldInterceptPaste = deps.shouldInterceptPaste;
		this.onPasteKey = deps.onPasteKey;
	...
	/** Intercept the platform paste key (app.clipboard.pasteImage: ctrl+v on
	 *  macOS/Linux, alt+v on Windows) BEFORE pi's built-in paste handler when
	 *  `clipboardPaste` is on, routing it through the marker-based clipboard
	 *  paste instead. This replaces the ctrl+v registerShortcut — registering it
	 *  made pi report an "Extension shortcut conflict" for overriding its
	 *  built-in paste binding; intercepting in the editor avoids the conflict
	 *  entirely. When off (or the key isn't the paste key), falls through to
	 *  {@link CustomEditor.handleInput}, which runs extension shortcuts (super+v,
	 *  Command+V on macOS), then pi's built-in paste, then normal editing.
	 *  Command+V on macOS never reaches the editor (terminals consume it for
	 *  their own paste), so it cannot be intercepted here. */
	override handleInput(data: string): void {
		if (this.shouldInterceptPaste() && this.keybindingsManager.matches(data, "app.clipboard.pasteImage")) {
			this.onPasteKey(this);
			return;
		}
