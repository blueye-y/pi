...
export function willBeResized(buf: Buffer, mimeType: string): boolean {
	...
	if (!dims) return true;
	return (
		dims.width > RESIZE_MAX_WIDTH || dims.height > RESIZE_MAX_HEIGHT || base64Size(buf.length) >= RESIZE_MAX_BYTES
	);
}
...
export type ResizeFn = (inputBytes: Uint8Array, mimeType: string) => Promise<{ data: string; mimeType: string } | null>;
...
const LEADING_WRAP_RE = /^[^A-Za-z0-9_/@.~-]+/;
