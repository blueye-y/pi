...
export class VisionModelSelectorComponent implements Component {
	...
	render(width: number): string[] {
		...
		lines.push("");
		lines.push(truncateToWidth(this.theme.fg("accent", this.theme.bold("Vision Handoff")), width, ""));
		lines.push(
			...wrapTextWithAnsi(
	...
	private getFooterText(): string {
		...
		const current = this.currentRef ? `current: ${this.currentRef}` : "current: none";
		...
		const parts: string[] = [
			...
			this.searchInput.getValue()
				...
				: `${totalCount} models · ${visionCount} vision`,
		];
	...
	private updateList(): void {
		...
		for (let i = startIndex; i < endIndex; i++) {
			...
			if (item.none) {
				...
				const labelled = isSelected ? this.theme.fg("accent", item.modelId) : item.modelId;
			...
			const current =
				item.ref === this.currentRef && item.ref !== null
		...
		if (selected) {
			...
			if (selected.none) {
				this.listContainer.addChild(new Text(this.theme.fg("muted", `  ${selected.modelName}`), 0, 0));
			} else {
				this.listContainer.addChild(new Text(this.theme.fg("muted", `  Model Name: ${selected.modelName}`), 0, 0));
				this.listContainer.addChild(
					new Text(
						this.theme.fg(
							"dim",
			...
			const fallback = this.asyncClipboardHandoff ? this.theme.fg("success", "on") : this.theme.fg("muted", "off");
			this.listContainer.addChild(new Text(this.theme.fg("dim", `  Async pasted-path fallback: ${fallback}`), 0, 0));
		}
	...
	private renderThinkingDetail(selected: DisplayItem): void {
		...
		this.listContainer.addChild(new Text(this.theme.fg("dim", `  Thinking: ${state}`), 0, 0));
		if (this.thinking && !selected.none && !selected.reasoning) {
			this.listContainer.addChild(
				new Text(
					this.theme.fg("warning", `  ⚠ ${selected.modelId} declares no reasoning — thinking will be ignored`),
					0,
					0,
				),
			);
