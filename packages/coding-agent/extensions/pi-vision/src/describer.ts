...
import { type AbortWire, abortWireGuard, fetchInterceptorGuard, timeoutGuard } from "./dispose.js";
...
import {
	...
	type VisionEnergyCapture,
	type VisionUsageRecord,
} from "./usage.js";
...
export function resolveMaxTokens(cfg: VisionConfig, visionModel: Model<Api>): number | undefined {
 ...
 *  - Otherwise returns the configured {@link VisionConfig.thinkingLevel}.
 *
 *  Mirrors pi core's `createSummarizationOptions` guard so the describer's
 *  thinking behaviour matches the rest of pi. */
export function resolveReasoning(cfg: VisionConfig, visionModel: Model<Api>): ThinkingLevel | undefined {
...
function configSnapshot(cfg: VisionConfig): {
	maxTokens?: number;
	thinking: boolean;
...
function isolateVisionRequest(model: Model<Api>, options: SimpleStreamOptions): SimpleStreamOptions {
	const sessionId = `pi-vision:${randomUUID()}`;
...
export interface DescriberDeps {
	...
	reportUsage(record: VisionUsageRecord): void;
...
export async function runBatch(
	...
	let timedOut = false;
	using _fetchGuard = fetchInterceptorGuard();
	using _timer = timeoutGuard(timeoutMs, () => {
		timedOut = true;
		controller.abort();
	});
...
async function readCapture(describeCtx: DescribeContext): Promise<VisionEnergyCapture> {
