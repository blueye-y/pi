/**
 * Shared constants, types, and utilities for pi-vision.
 *
 * Config lives at ~/.pi/agent/extensions/pi-vision.json — the same
...
/** Config file name. */
export const CONFIG_FILENAME = "pi-vision.json";

/** Legacy config file name (pre-rename). Migrated to {@link CONFIG_FILENAME} on first read. */
export const LEGACY_CONFIG_FILENAME = "pi-vision-handoff.json";
/** Full config path: ~/.pi/agent/extensions/pi-vision.json */
...
export const HANDOFF_COMMAND_DESCRIPTION =
	"Configure vision — pick a vision model to describe images for text-only models";
 ...
 *  separately via {@link VisionConfig.thinking} (a boolean on/off
...
export interface VisionConfig {
	...
	/** Opt-in: override the paste shortcut (ctrl+v) to read clipboard images
	 *  directly and attach them as image blocks on submit (marker-based),
	 *  instead of pi's built-in temp-file-path paste. When off, the shortcut
	 *  still intercepts ctrl+v but replicates pi's built-in paste behaviour.
	 *  Integrated from @pi-archimedes/image-paste. Off by default. */
...
export function normalizeConfig(raw: unknown): VisionConfig {
	...
	if (typeof obj.clipboardPaste === "boolean") base.clipboardPaste = obj.clipboardPaste;
...
export function readConfig(): VisionConfig {
	...
	if (!existsSync(path)) {
		// Migrate a pre-rename config (pi-vision-handoff.json) to the new path
		// on first read, so an existing picker choice survives the rename.
		const legacyPath = join(resolveAgentDir(), CONFIG_SUBDIR, LEGACY_CONFIG_FILENAME);
		if (existsSync(legacyPath)) {
			try {
				const cfg = normalizeConfig(JSON.parse(readFileSync(legacyPath, "utf8")));
				writeConfig(cfg);
				return cfg;
			} catch {
...
export function writeConfig(config: VisionConfig): string {
	const path = getConfigPath();
...
export function batchUserPrompt(count: number, userPrompt: string, prefix: string): string {
	const userLine = userPrompt?.trim() ? `${prefix}${userPrompt}` : "";
	if (count <= 1) {
...
export function wrapDescription(description: string, cfg: VisionConfig): string {
	const { text: final } =
