/**
 * pi-vision — give text-only models vision by proxying image input
 * through a vision-capable model of your choice.
 *
 * Extracted from the GLM 5.1 vision pipeline in pi-umans-provider and
 ...
 * choice is persisted to ~/.pi/agent/extensions/pi-vision.json.
 *
 ...
 *
 * This file is the wiring layer: pi event handlers + the /vision
...
import { ClipboardPaste, getImagePasteShortcuts, type PasteContext } from "./src/paste.js";
import { PASTE_PREVIEW_ENTRY_TYPE, registerPastePreviewEntryRenderer } from "./src/preview.js";
import { USAGE_ENTRY_TYPE, USAGE_EVENT_CHANNEL, type VisionUsageRecord } from "./src/usage.js";
import type { VisionModelSelectorResult } from "./src/vision-model-selector.js";

let config: VisionConfig = readConfig();
...
function scheduleAsyncClipboardInjection(
	...
	void (async () => {
		const entries = (await Promise.all(prepared)).filter((entry): entry is PreparedClipboardImage => entry !== null);
		if (entries.length === 0) {
		...
		const content = [
			...
			...entries.map((entry, index) => `${entry.path}\n${descriptions[index] ?? UNAVAILABLE}`),
		].join("\n\n");
...
let reportUsage: (record: VisionUsageRecord) => void = () => {};
...
function isAnyVisionModelResolvable(modelRegistry: ModelRegistry, config: VisionConfig): boolean {
...
const getLoader = (): Promise<DescriptionLoader> => {
	loaderPromise ??= import("./src/dataloader.js").then(({ DescriptionLoader }) => new DescriptionLoader(loaderDeps));
	return loaderPromise;
};
const loadResizeImage = () => import("@earendil-works/pi-coding-agent").then(({ resizeImage }) => resizeImage);

function isConfigured(cfg: VisionConfig): boolean {
...
/** Prewarm a DIRECTLY pasted image (marker-based clipboard paste) through the
 *  dataloader at paste time. Same reset semantics as {@link prewarmClipboardPath}:
 *  bind only the model registry (no turn signal — the agent is idle at paste
 *  time) and reset the turn prompt to "" so a stale previous-turn prompt can't
 *  leak into the description. Unlike the path flow, the image bytes are already
 *  in hand (read from the clipboard), and the image block attached on submit
 *  carries the SAME bytes — so the description cache key matches the `context`
 *  hook's lookup. No file read or resize needed. */
function prewarmPastedImage(img: ExtractedImage, modelRegistry: ModelRegistry): void {
	if (!shouldPrewarmPaste()) return;
	void getLoader().then((loader) => {
		loader.resetTurnAbort();
		loader.setPendingTurnPrompt("");
		loader.bindTurnContext({ modelRegistry });
		loader.loadDescription(img).catch(() => {});
	...
	ctx.ui.setEditorComponent(
		(_tui, theme, keybindings) =>
			new PrewarmEditor(_tui, theme, keybindings, {
				...
				// Intercept the platform paste key (ctrl+v) in the editor instead of
				// registering it as a shortcut, so pi reports no "Extension shortcut
				// conflict". The editor inserts the marker at the cursor; notify goes
				// through the session-start ctx (ui.notify is a plain render call,
				// safe to keep across events).
				shouldInterceptPaste: () => config.clipboardPaste,
				onPasteKey: (editor) => {
					const target: PasteContext = {
						ui: {
							pasteToEditor: (text) => editor.insertTextAtCursor(text),
					...
					void clipboardPaste.paste(target);
				},
			}),
	);
}
...
// Wired in the factory below: shortcuts + input handler + preview entry
// renderer. `appendPastePreview` follows the reportUsage pattern — a module-
// level placeholder reassigned inside the factory so it always references the
// live pi (the factory re-runs on /new, /resume, fork, /reload).
let appendPastePreview: (images: ExtractedImage[]) => void = () => {};
const clipboardPaste = new ClipboardPaste({
	getConfig: () => config,
	prewarm: (img, modelRegistry) => prewarmPastedImage(img, modelRegistry),
	appendPreview: (images) => appendPastePreview(images),
...
function notifyUnresolvedVisionModel(ctx: ExtensionContext, ref: string): void {
	...
	if (ctx.hasUI) {
		ctx.ui.notify(
			`pi-vision: configured vision model "${ref}" was not found in the registry — run /vision to pick a model.`,
			"warning",
		);
	}
}
...
function warnFailedImages(ctx: ExtensionContext, imgs: ExtractedImage[], descs: string[], reason: string): void {
	...
	if (!ctx.hasUI) return;
	ctx.ui.notify(`pi-vision: image description failed — ${reason}. Vision model: ${config.visionModel}`, "warning");
}

export default function (pi: ExtensionAPI) {
	config = readConfig();
	registerPastePreviewEntryRenderer(pi);
	appendPastePreview = (images) => {
		try {
			pi.appendEntry(PASTE_PREVIEW_ENTRY_TYPE, { images });
		} catch {
			// never break the submit on persistence failure
		}
	};

	// Marker-based clipboard paste shortcuts (integrated from image-paste).
	// Registered ONLY when `clipboardPaste` is on: registering ctrl+v otherwise
	// would override pi's built-in paste-image keybinding and surface an
	// "Extension shortcut conflict" warning on every default install. When off,
	// ctrl+v stays pi's own built-in (image → temp file path), which is exactly
	// the flow the path-based prewarm expects. Toggling the config takes effect
	// on reload (pi re-runs this factory on /reload).
	if (config.clipboardPaste) {
		for (const shortcut of getImagePasteShortcuts()) {
			pi.registerShortcut(shortcut, {
				description: "Paste clipboard image (or text)",
				handler: (ctx) => clipboardPaste.paste(ctx),
			});
		}
	}

	// Attach queued pasted images as image blocks on submit: replace the
	// `[Image #N]` markers, strip them from the text, and let the existing
	// before_agent_start/context handoff describe the attached blocks.
	pi.on("input", (event) => clipboardPaste.onInput(event));

	pi.registerMessageRenderer("vision-async", (message, { expanded }, theme) => {
	...
	// the live pi.
	reportUsage = (record: VisionUsageRecord) => {
		try {
	...
	pi.on("before_agent_start", async (event, ctx) => {
		...
		const resizeImage = await loadResizeImage();
		const preparedClipboardImages = clipboardPaths.map(async (path): Promise<PreparedClipboardImage | null> => {
			try {
				const read = readImageBuffer(path);
				if (!read) return null;
	...
	pi.on("tool_result", async (event, ctx) => {
		...
		const omittedNoteIndices: number[] = [];
		if (!hasImageBlock) {
	...
	pi.on("model_select", (event, ctx) => {
		...
		if (isHandoffTarget(model, config) && !isVisionModel(model)) {
			ctx.ui.notify(`pi-vision: active — images will be described by ${config.visionModel}`, "info");
		}
	});
...
async function handleHandoffCommand(ctx: ExtensionCommandContext, args: string): Promise<void> {
	...
	// /vision (no args) or /vision select — interactive picker
	...
	if (subcommand === "help") {
		ctx.ui.notify(
			[
				"pi-vision commands:",
				"  /vision                 Open interactive picker to choose the vision model",
				"  /vision select         Same as /vision",
				"  /vision model <p/id>   Set the vision model directly",
				"  /vision status         Show current config and active state",
				"  /vision enable         Enable vision handoff",
				"  /vision disable        Disable vision handoff (keeps configured model)",
				"  /vision auto <on|off>  Toggle automatic handoff for all non-vision models",
				"  /vision thinking <off|minimal|low|medium|high|xhigh|max>",
				"                               Set the vision describer's thinking effort (off = disabled)",
				"  /vision prewarm <on|off>",
				"                               Toggle describing pasted images at paste-time (opt-in, off by default)",
				"  /vision fallback <on|off>",
				"                               Inject pasted-image descriptions asynchronously when no matching read wins",
				"  /vision paste             Paste the clipboard image into the editor now",
				"  /vision paste <on|off>  Toggle marker-based paste (intercepts ctrl+v in the",
				"                               editor; macOS Command+V if the terminal forwards it)",
				"  /vision add <p/id>     Force handoff for an extra model",
				"  /vision remove <p/id>  Stop forcing handoff for a model",
				"  /vision clear          Clear the configured vision model",
				"  /vision help           This message",
				"",
				"Config: ~/.pi/agent/extensions/pi-vision.json",
				...
				"  paste (no args) reads the clipboard image and inserts an [Image #N] marker.",
				"  paste on intercepts ctrl+v in the editor (no shortcut conflict warning);",
				"  Command+V also pastes when the terminal forwards it; image attaches on submit.",
			].join("\n"),
			"info",
		);
		return;
	...
	if (subcommand === "auto") {
		...
		if (value !== "on" && value !== "off") {
			ctx.ui.notify("Usage: /vision auto <on|off>", "warning");
			return;
		}
		const on = value === "on";
		updateConfig(
	...
	if (subcommand === "paste") {
		await handlePasteSubcommand(ctx, rest);
		return;
	}
	...
	if (subcommand === "model") {
		if (!rest) {
			ctx.ui.notify("Usage: /vision model <provider/id>", "warning");
			return;
		}
		...
		if (!model) {
			ctx.ui.notify(`Model not found: ${rest}. Use /vision to pick from the list.`, "error");
			return;
		}
	...
	if (subcommand === "add") {
		if (!rest) {
			ctx.ui.notify("Usage: /vision add <provider/id>", "warning");
			return;
		}
		const parsed = parseModelRef(rest);
		if (!parsed) {
	...
	if (subcommand === "remove") {
		if (!rest) {
			ctx.ui.notify("Usage: /vision remove <provider/id>", "warning");
			return;
		}
		const parsed = parseModelRef(rest);
		if (!parsed) {
	...
	ctx.ui.notify(`Unknown subcommand: "${subcommand}". Use /vision help for usage.`, "warning");
}

function updateConfig(
	ctx: ExtensionCommandContext,
	transform: (c: VisionConfig) => VisionConfig,
	message: string,
): void {
...
/** Resolve a `/vision thinking <level>` argument into a
...
function handleThinkingSubcommand(ctx: ExtensionCommandContext, rest: string): void {
	...
	if (!arg) {
		ctx.ui.notify(
			`Thinking: ${config.thinking ? `on (${config.thinkingLevel})` : "off"}.\n` +
				`Usage: /vision thinking <off|minimal|low|medium|high|xhigh|max>`,
			"info",
		);
		return;
	}
	...
	if (!isThinkingLevel(arg)) {
		ctx.ui.notify(`Unknown thinking level: "${arg}". Use off, minimal, low, medium, high, xhigh, or max.`, "error");
		return;
	}
	const level = arg;
	updateConfig(
...
function handlePrewarmSubcommand(ctx: ExtensionCommandContext, rest: string): void {
	...
	const note = on
		? editorInstalled
			...
			: "Paste-time prewarm on — but another custom editor extension is active, so it's unavailable. Submit-time prewarm still works; disable the other editor extension (or /vision prewarm off) to silence this."
		: "Paste-time prewarm off — images are described at submit time (default).";
	updateConfig(ctx, (c) => ({ ...c, prewarmPastedImages: on }), note);
}

/** Handle /vision fallback <on|off>. */
	...
	if (!value) {
		ctx.ui.notify(
			`Async pasted-path fallback: ${config.asyncClipboardHandoff ? "on" : "off"}.\n` +
				"Usage: /vision fallback <on|off>",
			"info",
		);
		return;
	}
	if (value !== "on" && value !== "off") {
		ctx.ui.notify("Usage: /vision fallback <on|off>", "warning");
		return;
	}
	const on = value === "on";
	updateConfig(
...
/** Handle /vision paste — with no args, paste the clipboard image into the editor
 *  now (keybinding-free fallback for terminals that swallow ctrl+v). With
 *  <on|off>, toggle marker-based clipboard paste: the paste shortcuts are
 *  registered by the extension factory (config-gated), so the toggle is
 *  followed by an automatic reload — pi re-runs the factories and the
 *  shortcuts come alive immediately. The old ctx is stale after the
 *  reload, so nothing else may use it. */
async function handlePasteSubcommand(ctx: ExtensionCommandContext, rest: string): Promise<void> {
	const value = rest.trim().toLowerCase();
	if (!value) {
		// /vision paste — paste the clipboard image into the editor right now.
		// A keybinding-free fallback: terminals (e.g. Orca's embedded one) may
		// intercept ctrl+v at the terminal layer so pi never sees the keypress.
		await clipboardPaste.paste(ctx);
		return;
	}
	if (value !== "on" && value !== "off") {
		ctx.ui.notify("Usage: /vision paste <on|off>", "warning");
		return;
	}
	const on = value === "on";
	updateConfig(
		ctx,
		(c) => ({ ...c, clipboardPaste: on }),
		on
			? "Marker-based clipboard paste on — reloading so the ctrl+v shortcut takes effect."
			: "Marker-based clipboard paste off — reloading to restore pi's built-in ctrl+v paste.",
	);
	// Reload now so the config-gated shortcut registration takes effect without
	// a manual /reload. While the agent is streaming/compacting the reload is
	// skipped (pi shows its own warning) and the toggle applies on the next
	// reload; a thrown reload may leave the ctx stale, so the failure notify is
	// best-effort.
	try {
		await ctx.reload();
	} catch (error) {
		try {
			...
			ctx.ui.notify(`Reload skipped (${msg}) — the change applies on the next reload.`, "warning");
		} catch {
			// ctx may be stale after a partial reload — swallow.
		}
	}
}
...
async function showSelector(ctx: ExtensionCommandContext): Promise<void> {
	if (!ctx.hasUI) {
		ctx.ui.notify("/vision requires interactive mode.", "error");
		return;
	}
...
function showStatus(ctx: ExtensionCommandContext): void {
	...
	lines.push(`Vision model: ${config.visionModel ?? "(none — pick one with /vision)"}`);
	...
	lines.push(
		`Paste-time prewarm: ${config.prewarmPastedImages ? `on${editorInstalled ? "" : " (inactive — another custom editor is active)"}` : "off"}`,
	);
	lines.push(`Async pasted-path fallback: ${config.asyncClipboardHandoff ? "on" : "off"}`);
	lines.push(`Marker-based clipboard paste (ctrl+v): ${config.clipboardPaste ? "on" : "off"}`);
	lines.push(
