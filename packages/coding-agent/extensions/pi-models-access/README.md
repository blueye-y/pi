...
- **Footer status lines**: DeepSeek shows your account balance in the footer (`DS ¥12.34`, refreshed after each completed conversation round and on a 15-minute timer — toggle with `/deepseek → Balance in status bar`), **only while the active model is a DeepSeek model** (switching to another provider hides it, same as the Alibaba quota line); Alibaba Cloud shows the active model's rate-limit quota when the limits endpoint is reachable (Beijing workspace domain).
